
<html>
<body>
<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,databaseconnection.*" errorPage="" %>
<%
      
	 
        String uname=request.getParameter("uname");
         System.out.println("uname="+uname);
        String pword=request.getParameter("password");
        System.out.println("password="+pword);
		
		
         
	    try{
			
	     Connection con = databasecon.getconnection();
         PreparedStatement ps=con.prepareStatement("select *  from customer_register where uname='"+uname+"' and password='"+pword+"'");
	     ResultSet rs=ps.executeQuery();
		 if(rs.next())
		 { 
		
		 
		 int n=rs.getInt("id");
		 String nn=String.valueOf(n);
		 session.setAttribute("cid",nn);
		 System.out.println("username"+nn);
		 String p=rs.getString("uname");
		 session.setAttribute("cname",p);
		 System.out.println("username"+p);
		 // String m=rs.getString("email");
		// session.setAttribute("bb",email);
		// System.out.println("email id"+bb);
		 String email1=rs.getString("email");
		 session.setAttribute("email1",email1);
		   
	     response.sendRedirect("customer_home.jsp");
		
		 }
		 else
		 {%>
		 <script>alert("incorrect username and password");
		 window.location="customer.jsp";
		 </script>
	<%
		 }
		

	}
	catch(Exception e2){
		out.println(e2.getMessage());
	}
	
	
%>
</body>
</html>